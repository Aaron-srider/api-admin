                    ...
                    <div v-if="scope.col.prop == undefined">
                        <div>
                            <el-button
                                circle
                                icon="el-icon-delete"
                                @click="remove_task(scope.row.id)"
                            ></el-button>

                            <el-button circle @click="into_info(scope.row.id)">
                                <i
                                    class="fs16 ali-international-icon-jurassic_log"
                                ></i>
                            </el-button>
                        </div>
                    </div>
                ...
                <template slot-scope="scope">
                    <div class="flex column justify-between">
                        ...
                        <div class="mgt10">
                            ...
                            <div class="mgt10">
                                <el-input v-model="scope.data.cron"></el-input>
                            </div>
                        </div>
                    </div>
                </template>
...
import { StringUtils, to_page, when_unknow_err } from '@/utils';
import { Message } from 'element-ui';

import { create_db_conn, get_db_conns } from '@/api/db_conn';
import { create_task, get_tasks, remove_task } from '@/api/task';
let vue;
export default {
    ...
    methods: {
        into_info(task_id) {
            to_page(this.$router, '/db-auto-backup/log', {
                task_id,
            });
        },
        remove_task(task_id) {
